...
-- Trigger - notify new procurement order
DELIMITER //

CREATE TRIGGER NewProcurementOrderTrigger
AFTER INSERT ON Procurement
FOR EACH ROW
BEGIN
    -- Send a message using MySQL's SIGNAL statement
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'A new procurement order has been created!';
END //

DELIMITER ;

-- Trigger - notify finish procurement order
DELIMITER //

CREATE TRIGGER ProcurementProcessedTrigger
AFTER UPDATE ON procurement
FOR EACH ROW
BEGIN
    -- Check if the "processed" column has changed from 0 to 1
    IF OLD.processed = 0 AND NEW.processed = 1 THEN
        -- Send a message using MySQL's SIGNAL statement
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'The procurement order has been processed!';
    END IF;
END //

DELIMITER ;

-- Dropping the triggers because they are useless and I don't like it
Drop TRIGGER NewProcurementOrderTrigger;
DROP TRIGGER ProcurementProcessedTrigger;
